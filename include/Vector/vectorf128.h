...
static finline __m128 selectf (__m128 const & s, __m128 const & a, __m128 const & b) {
...
// otherwise all bits in s are used.
static finline __m128d selectd (__m128d const & s, __m128d const & a, __m128d const & b) {
#if INSTRSET >= 5   // SSE4.1 supported
...
public:
    ...
    finline Vec4fb(bool b) {
        xmm = _mm_castsi128_ps(_mm_set1_epi32(-(int)b));
    };
    // Constructor to build from all elements:
    finline Vec4fb(bool b0, bool b1, bool b2, bool b3) {
...
static finline Vec4fb operator & (Vec4fb const & a, Vec4fb const & b) {
...
// vector operator &= : bitwise and
static finline Vec4fb & operator &= (Vec4fb & a, Vec4fb const & b) {
    a = a & b;
    return a;
}
...
static finline Vec4fb operator ~ (Vec4fb const & a) {
...
// andnot: a & ~ b
static finline Vec4fb andnot(Vec4fb const & a, Vec4fb const & b) {
...
static finline bool horizontal_and (Vec4fb const & a) {
    return horizontal_and(Vec128b(_mm_castps_si128(a)));
}

// horizontal_or. Returns true if at least one bit is 1
static finline bool horizontal_or (Vec4fb const & a) {
...
public:
    ...
    // Constructor to broadcast the same value into all elements:
    finline Vec2db(bool b) {
        xmm = _mm_castsi128_pd(_mm_set1_epi32(-(int)b));
    };
    // Constructor to build from all elements:
    finline Vec2db(bool b0, bool b1) {
    ...
    finline Vec2db(__m128d const & x) {
        xmm = x;
    };
    // Assignment operator to convert from type __m128d used in intrinsics:
    finline Vec2db & operator = (__m128d const & x) {
        xmm = x;
        return *this;
    };
    // Type cast operator to convert to __m128d used in intrinsics
    finline operator __m128d() const {
        return xmm;
    }
    // Constructor to convert from type Vec2q used as Boolean for integer vectors
    finline Vec2db(Vec2q const & x) {
        xmm = _mm_castsi128_pd(x);
    };
    // Assignment operator to convert from type Vec2q used as Boolean for integer vectors
    finline Vec2db & operator = (Vec2q const & x) {
        xmm = _mm_castsi128_pd(x);
        return *this;
    };
    // Type cast operator to convert to type Vec2q used as Boolean for integer vectors
    finline operator Vec2q() const {
        return _mm_castpd_si128(xmm);
    }
    // Member function to change a single element in vector
    // Note: This function is inefficient. Use load function if changing more than one element
    finline Vec2db const & insert(uint32_t index, bool value) {
...
// vector operator & : bitwise and
static finline Vec2db operator & (Vec2db const & a, Vec2db const & b) {
...
// vector operator &= : bitwise and
static finline Vec2db & operator &= (Vec2db & a, Vec2db const & b) {
    a = a & b;
    return a;
}
...
// vector operator ~ : bitwise not
static finline Vec2db operator ~ (Vec2db const & a) {
...
// andnot: a & ~ b
static finline Vec2db andnot(Vec2db const & a, Vec2db const & b) {
...
// horizontal_and. Returns true if all bits are 1
static finline bool horizontal_and (Vec2db const & a) {
    return horizontal_and(Vec128b(_mm_castpd_si128(a)));
}

// horizontal_or. Returns true if at least one bit is 1
static finline bool horizontal_or (Vec2db const & a) {
...
public:
    ...
    // Constructor to broadcast the same value into all elements:
    finline Vec4f(float f) {
        xmm = _mm_set1_ps(f);
    }
    // Constructor to build from all elements:
    finline Vec4f(float f0, float f1, float f2, float f3) {
        xmm = _mm_setr_ps(f0, f1, f2, f3);
    }
    // Constructor to convert from type __m128 used in intrinsics:
    finline Vec4f(__m128 const & x) {
        xmm = x;
    }
    // Assignment operator to convert from type __m128 used in intrinsics:
    finline Vec4f & operator = (__m128 const & x) {
        xmm = x;
        return *this;
    }
    // Type cast operator to convert to __m128 used in intrinsics
    ...
    finline Vec4f & load(float const * p) {
        xmm = _mm_loadu_ps(p);
        return *this;
    }
    ...
    // divisible by 16.
    finline Vec4f & load_a(float const * p) {
        xmm = _mm_load_ps(p);
        return *this;
    }
    finline Vec4f & load_s(float const * p) {
        __m128i temp = _mm_stream_load_si128((__m128i*)p);
    ...
    finline Vec4f & load_partial(int n, float const * p) {
        __m128 t1, t2;
        switch (n) {
        case 1:
    ...
    // Member function to change a single element in vector
    // Note: This function is inefficient. Use load function if changing more than one element
    finline Vec4f const & insert(uint32_t index, float value) {
#if INSTRSET >= 5   // SSE4.1 supported
        switch (index & 3) {
        case 0:
...
static finline Vec4f operator + (Vec4f const & a, Vec4f const & b) {
    return _mm_add_ps(a, b);
}

// vector operator + : add vector and scalar
static finline Vec4f operator + (Vec4f const & a, float b) {
    return a + Vec4f(b);
}
static finline Vec4f operator + (float a, Vec4f const & b) {
    return Vec4f(a) + b;
}

// vector operator += : add
static finline Vec4f & operator += (Vec4f & a, Vec4f const & b) {
    a = a + b;
    return a;
}

// postfix operator ++
static finline Vec4f operator ++ (Vec4f & a, int) {
    Vec4f a0 = a;
    a = a + 1.0f;
    return a0;
}
...
// prefix operator ++
static finline Vec4f & operator ++ (Vec4f & a) {
    a = a + 1.0f;
    return a;
}
...
static finline Vec4f operator - (Vec4f const & a) {
...
static finline Vec4fb operator == (Vec4f const & a, Vec4f const & b) {
...
static finline Vec4fb operator ! (Vec4f const & a) {
    return a == 0.0f;
}
...
static finline Vec4f select (Vec4fb const & s, Vec4f const & a, Vec4f const & b) {
...
static finline float horizontal_add (Vec4f const & a) {
#if  INSTRSET >= 3  // SSE3
...
static finline Vec4f max(Vec4f const & a, Vec4f const & b) {
    return _mm_max_ps(a,b);
}

// function min: a < b ? a : b
static finline Vec4f min(Vec4f const & a, Vec4f const & b) {
    return _mm_min_ps(a,b);
}

// function abs: absolute value
// Removes sign bit, even for -0.0f, -INF and -NAN
static finline Vec4f abs(Vec4f const & a) {
    __m128 mask = _mm_castsi128_ps(_mm_set1_epi32(0x7FFFFFFF));
    return _mm_and_ps(a,mask);
}

// function sqrt: square root
static finline Vec4f sqrt(Vec4f const & a) {
    return _mm_sqrt_ps(a);
}

// function square: a * a
static finline Vec4f square(Vec4f const & a) {
    return a * a;
}

// pow(Vec4f, int):
// Raise floating point numbers to integer power n
static finline Vec4f pow(Vec4f const & a, int n) {
    Vec4f x = a;                       // a^(2^i)
...
template <int n>
static finline Vec4f pow_n(Vec4f const & a) {
...
template <int n>
static finline Vec4f pow(Vec4f const & a, Const_int_t<n>) {
    return pow_n<n>(a);
}


// avoid unsafe optimization in function round
#if defined(__GNUC__) && !defined(__INTEL_COMPILER) && INSTRSET < 5
static finline Vec4f round(Vec4f const & a) __attribute__ ((optimize("-fno-unsafe-math-optimizations")));
...
static finline Vec4f round(Vec4f const & a) {
#if INSTRSET >= 5   // SSE4.1 supported
...
#if (defined (_MSC_VER) || defined(__INTEL_COMPILER)) && INSTRSET < 5
#pragma float_control(pop)
#endif

// function truncate: round towards zero. (result as float vector)
static finline Vec4f truncate(Vec4f const & a) {
#if INSTRSET >= 5   // SSE4.1 supported
...
static finline Vec4f floor(Vec4f const & a) {
#if INSTRSET >= 5   // SSE4.1 supported
...
static finline Vec4f ceil(Vec4f const & a) {
#if INSTRSET >= 5   // SSE4.1 supported
...
static finline Vec4i round_to_int(Vec4f const & a) {
    // Note: assume MXCSR control register is set to rounding
    return _mm_cvtps_epi32(a);
}

// function truncate_to_int: round towards zero. (result as integer vector)
static finline Vec4i truncate_to_int(Vec4f const & a) {
    return _mm_cvttps_epi32(a);
}

// function to_float: convert integer vector to float vector
static finline Vec4f to_float(Vec4i const & a) {
...
static finline Vec4f approx_recipr(Vec4f const & a) {
    return _mm_rcp_ps(a);
}

// approximate reciprocal squareroot (Faster than 1.f / sqrt(a). Relative accuracy better than 2^-11)
static finline Vec4f approx_rsqrt(Vec4f const & a) {
...
static finline Vec4i exponent(Vec4f const & a) {
...
static finline Vec4f fraction(Vec4f const & a) {
    Vec4ui t1 = _mm_castps_si128(a);   // reinterpret as 32-bit integer
...
static finline Vec4f exp2(Vec4i const & n) {
...
static finline uint32_t get_control_word() {
    return _mm_getcsr();
}

// Function set_control_word:
// Write the MXCSR control word
static finline void set_control_word(uint32_t w) {
    _mm_setcsr(w);
}
...
static finline void no_denormals() {
...
// and allow denormals.
static finline void reset_control_word() {
...
static finline Vec4fb sign_bit(Vec4f const & a) {
...
// (the underscore in the name avoids a conflict with a macro in Intel's mathimf.h)
static finline Vec4fb is_finite(Vec4f const & a) {
    Vec4i t1 = _mm_castps_si128(a);    // reinterpret as 32-bit integer
...
// (the underscore in the name avoids a conflict with a macro in Intel's mathimf.h)
static finline Vec4fb is_inf(Vec4f const & a) {
    Vec4i t1 = _mm_castps_si128(a);    // reinterpret as 32-bit integer
    Vec4i t2 = t1 << 1;                // shift out sign bit
...
static finline Vec4f permute4f(Vec4f const & a) {
    // is shuffling needed
...
// blend vectors Vec4f
template <int i0, int i1, int i2, int i3>
static finline Vec4f blend4f(Vec4f const & a, Vec4f const & b) {
...
template <int i0, int i1, int i2, int i3>
static finline Vec4f change_sign(Vec4f const & a) {
...
public:
    ...
    // Constructor to broadcast the same value into all elements:
    finline Vec2d(double d) {
        xmm = _mm_set1_pd(d);
    }
    // Constructor to build from all elements:
    finline Vec2d(double d0, double d1) {
        xmm = _mm_setr_pd(d0, d1);
    }
    // Constructor to convert from type __m128d used in intrinsics:
    finline Vec2d(__m128d const & x) {
        xmm = x;
    }
    // Assignment operator to convert from type __m128d used in intrinsics:
    finline Vec2d & operator = (__m128d const & x) {
        xmm = x;
        return *this;
    }
    // Type cast operator to convert to __m128d used in intrinsics
    ...
    // Member function to load from array (unaligned)
    finline Vec2d & load(double const * p) {
        xmm = _mm_loadu_pd(p);
        return *this;
    }
    // Member function to load from array, aligned by 16
    ...
    // You may use load_a instead of load if you are certain that p points to an address
    // divisible by 16.
    finline Vec2d const & load_a(double const * p) {
        xmm = _mm_load_pd(p);
        return *this;
    }
    finline Vec2d const & load_s(double const * p) {
        __m128i temp = _mm_stream_load_si128((__m128i*)p);
    ...
    // Partial load. Load n elements and set the rest to 0
    finline Vec2d & load_partial(int n, double const * p) {
        if (n == 1) {
            xmm = _mm_load_sd(p);
        }
        else if (n == 2) {
    ...
    // cut off vector to n elements. The last 4-n elements are set to zero
    finline Vec2d & cutoff(int n) {
        xmm = _mm_castps_pd(Vec4f(_mm_castpd_ps(xmm)).cutoff(n*2));
        return *this;
    }
    // Member function to change a single element in vector
    // Note: This function is inefficient. Use load function if changing more than one element
    finline Vec2d const & insert(uint32_t index, double value) {
...
// vector operator + : add element by element
static finline Vec2d operator + (Vec2d const & a, Vec2d const & b) {
    return _mm_add_pd(a, b);
}

// vector operator + : add vector and scalar
static finline Vec2d operator + (Vec2d const & a, double b) {
    return a + Vec2d(b);
}
static finline Vec2d operator + (double a, Vec2d const & b) {
    return Vec2d(a) + b;
}

// vector operator += : add
static finline Vec2d & operator += (Vec2d & a, Vec2d const & b) {
    a = a + b;
    return a;
}

// postfix operator ++
static finline Vec2d operator ++ (Vec2d & a, int) {
    Vec2d a0 = a;
    a = a + 1.0;
    return a0;
}
...
// prefix operator ++
static finline Vec2d & operator ++ (Vec2d & a) {
    a = a + 1.0;
    return a;
}
...
// vector operator - : unary minus
// Change sign bit, even for 0, INF and NAN
static finline Vec2d operator - (Vec2d const & a) {
...
// vector operator == : returns true for elements for which a == b
static finline Vec2db operator == (Vec2d const & a, Vec2d const & b) {
...
// vector operator ! : logical not. Returns Boolean vector
static finline Vec2db operator ! (Vec2d const & a) {
    return a == 0.0;
}
...
static finline Vec2d select (Vec2db const & s, Vec2d const & a, Vec2d const & b) {
...
// Horizontal add: Calculates the sum of all vector elements.
static finline double horizontal_add (Vec2d const & a) {
#if  INSTRSET >= 3  // SSE3
...
// function max: a > b ? a : b
static finline Vec2d max(Vec2d const & a, Vec2d const & b) {
    return _mm_max_pd(a,b);
}

// function min: a < b ? a : b
static finline Vec2d min(Vec2d const & a, Vec2d const & b) {
    return _mm_min_pd(a,b);
}

// function abs: absolute value
// Removes sign bit, even for -0.0f, -INF and -NAN
static finline Vec2d abs(Vec2d const & a) {
    __m128d mask = _mm_castsi128_pd(_mm_setr_epi32(-1,0x7FFFFFFF,-1,0x7FFFFFFF));
    return _mm_and_pd(a,mask);
}

// function sqrt: square root
static finline Vec2d sqrt(Vec2d const & a) {
    return _mm_sqrt_pd(a);
}

// function square: a * a
static finline Vec2d square(Vec2d const & a) {
    return a * a;
}

// pow(Vec2d, int):
// Raise floating point numbers to integer power n
static finline Vec2d pow(Vec2d const & a, int n) {
    Vec2d x = a;                       // a^(2^i)
...
// Raise floating point numbers to integer power n, where n is a compile-time constant
template <int n>
static finline Vec2d pow_n(Vec2d const & a) {
...
template <int n>
static finline Vec2d pow(Vec2d const & a, Const_int_t<n>) {
    return pow_n<n>(a);
}
...
static finline Vec2d round(Vec2d const & a) {
#if INSTRSET >= 5   // SSE4.1 supported
...
static finline Vec2d truncate(Vec2d const & a) {
...
// (note: may fail on MS Visual Studio 2008, works in later versions)
static finline Vec2d floor(Vec2d const & a) {
#if INSTRSET >= 5   // SSE4.1 supported
...
static finline Vec2d ceil(Vec2d const & a) {
#if INSTRSET >= 5   // SSE4.1 supported
...
// function round_to_int: round to nearest integer (even). (result as integer vector)
static finline Vec4i round_to_int(Vec2d const & a, Vec2d const & b) {
    // Note: assume MXCSR control register is set to rounding
...
static finline Vec4i truncate_to_int(Vec2d const & a, Vec2d const & b) {
...
static finline Vec2q truncate_to_int64(Vec2d const & a) {
    double aa[2];
    a.store(aa);
...
static finline Vec2d to_double(Vec2q const & a) {
    int64_t aa[2];
    a.store(aa);
...
static finline Vec2d extend_low (Vec4f const & a) {
    return _mm_cvtps_pd(a);
}

// Function extend_high : convert Vec4f vector elements [2] and [3] to Vec2d
static finline Vec2d extend_high (Vec4f const & a) {
...
static finline Vec2q exponent(Vec2d const & a) {
...
static finline Vec2d fraction(Vec2d const & a) {
    Vec2uq t1 = _mm_castpd_si128(a);   // reinterpret as 64-bit integer
...
// n <= -1023 gives 0.0
// This function will never produce denormals, and never raise exceptions
static finline Vec2d exp2(Vec2q const & n) {
...
static finline Vec2db sign_bit(Vec2d const & a) {
...
// Function is_finite: gives true for elements that are normal, denormal or zero,
// false for INF and NAN
static finline Vec2db is_finite(Vec2d const & a) {
    ...
    Vec2q t4 = Vec2q(t2 & t3) != t3;   // exponent field is not all 1s
    return t4;
}

// Function is_inf: gives true for elements that are +INF or -INF
// false for finite numbers and NAN
static finline Vec2db is_inf(Vec2d const & a) {
    Vec2q t1 = _mm_castpd_si128(a);    // reinterpret as 32-bit integer
    Vec2q t2 = t1 << 1;                // shift out sign bit
...
// Function is_nan: gives true for elements that are +NAN or -NAN
// false for finite numbers and +/-INF
static finline Vec2db is_nan(Vec2d const & a) {
    Vec2q t1 = _mm_castpd_si128(a);    // reinterpret as 32-bit integer
    Vec2q t2 = t1 << 1;                // shift out sign bit
    Vec2q t3 = 0xFFE0000000000000ll;   // exponent mask
    Vec2q t4 = t2 & t3;                // exponent
    Vec2q t5 = _mm_andnot_si128(t3,t2);// fraction
    return Vec2q((t4==t3) & (t5!=0));  // exponent = all 1s and fraction != 0
}

// Function is_denormal: gives true for elements that are denormal (subnormal)
// false for finite numbers, zero, NAN and INF
static finline Vec2db is_denormal(Vec2d const & a) {
    Vec2q t1 = _mm_castpd_si128(a);    // reinterpret as 32-bit integer
    Vec2q t2 = t1 << 1;                // shift out sign bit
    Vec2q t3 = 0xFFE0000000000000ll;   // exponent mask
    Vec2q t4 = t2 & t3;                // exponent
    Vec2q t5 = _mm_andnot_si128(t3,t2);// fraction
    return Vec2q((t4==0) & (t5!=0));   // exponent = 0 and fraction != 0
}

// Function infinite2d: returns a vector where all elements are +INF
static finline Vec2d infinite2d() {
...
static finline __m128i reinterpret_i (__m128i const & x) {
    return x;
}

static finline __m128i reinterpret_i (__m128  const & x) {
    return _mm_castps_si128(x);
}

static finline __m128i reinterpret_i (__m128d const & x) {
    return _mm_castpd_si128(x);
}

static finline __m128  reinterpret_f (__m128i const & x) {
    return _mm_castsi128_ps(x);
}

static finline __m128  reinterpret_f (__m128  const & x) {
    return x;
}

static finline __m128  reinterpret_f (__m128d const & x) {
    return _mm_castpd_ps(x);
}

static finline __m128d reinterpret_d (__m128i const & x) {
    return _mm_castsi128_pd(x);
}

static finline __m128d reinterpret_d (__m128  const & x) {
    return _mm_castps_pd(x);
}

static finline __m128d reinterpret_d (__m128d const & x) {
    return x;
}
...
static finline Vec2d permute2d(Vec2d const & a) {
    // is shuffling needed
...
// blend vectors Vec2d
template <int i0, int i1>
static finline Vec2d blend2d(Vec2d const & a, Vec2d const & b) {
...
template <int i0, int i1>
static finline Vec2d change_sign(Vec2d const & a) {
    if ((i0 | i1) == 0) return a;
...
static finline Vec4f lookup4(Vec4i const & index, Vec4f const & table) {
#if INSTRSET >= 7  // AVX
...
static finline Vec4f lookup8(Vec4i const & index, Vec4f const & table0, Vec4f const & table1) {
#if INSTRSET >= 8  // AVX2
...
template <int n>
static finline Vec4f lookup(Vec4i const & index, float const * table) {
    if (n <= 0) return 0.0f;
...
static finline Vec2d lookup2(Vec2q const & index, Vec2d const & table) {
#if INSTRSET >= 7  // AVX
...
static finline Vec2d lookup4(Vec2q const & index, Vec2d const & table0, Vec2d const & table1) {
#if INSTRSET >= 7  // AVX
...
template <int n>
static finline Vec2d lookup(Vec2q const & index, double const * table) {
    if (n <= 0) return 0.0;
