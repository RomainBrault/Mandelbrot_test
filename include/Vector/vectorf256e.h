...
class Vec256fe {
public:
    finline Vec256fe(void) {};                 // default constructor
    finline Vec256fe(__m128 x0, __m128 x1) {   // constructor to build from two __m128
        y0 = x0;  y1 = x1;
    }
    finline __m128 get_low() const {           // get low half
        return y0;
    }
    finline __m128 get_high() const {          // get high half
        return y1;
    }
protected:
    __m128 y0;                         // low half
...
class Vec256de {
public:
    finline Vec256de() {};                     // default constructor
    finline Vec256de(__m128d x0, __m128d x1) { // constructor to build from two __m128d
        y0 = x0;  y1 = x1;
    }
    finline __m128d get_low() const {          // get low half
        return y0;
    }
    finline __m128d get_high() const {         // get high half
        return y1;
    }
protected:
    __m128d y0;                        // low half
...
static finline Vec256fe selectf (Vec256fe const & s, Vec256fe const & a, Vec256fe const & b) {
...
// values are allowed.
static finline Vec256de selectd (Vec256de const & s, Vec256de const & a, Vec256de const & b) {
...
static finline Vec256fe constant8f() {
    static const union {
        int      i[8];
        __m128   y[2];
...
public:
    ...
    finline Vec4db(Vec256de const & x) {
        y0 = x.get_low();  y1 = x.get_high();
    }
    // Assignment operator to convert from type Vec256de
    finline Vec4db & operator = (Vec256de const & x) {
        y0 = x.get_low();  y1 = x.get_high();
        return *this;
    }
#ifdef VECTORI256_H  // 256 bit integer vectors are available
...
public:
    ...
    // Constructor to convert from type Vec256fe
    finline Vec8f(Vec256fe const & x) {
        y0 = x.get_low();  y1 = x.get_high();
    }
    // Assignment operator to convert from type Vec256fe
    finline Vec8f & operator = (Vec256fe const & x) {
        y0 = x.get_low();  y1 = x.get_high();
        return *this;
    }
    ...
    finline Vec8f & load_s(double const * p) {
...
static finline Vec8f exp2(Vec8i const & a) {
...
public:
    ...
    // Constructor to convert from type Vec256de
    finline Vec4d(Vec256de const & x) {
        y0 = x.get_low();
        y1 = x.get_high();
    }
    // Assignment operator to convert from type Vec256de
    finline Vec4d & operator = (Vec256de const & x) {
        y0 = x.get_low();
        y1 = x.get_high();
        return *this;
    }
...
// n <= -1023 gives 0.0
// This function will never produce denormals, and never raise exceptions
static finline Vec4d exp2(Vec4q const & a) {
...
static finline Vec256ie reinterpret_i (Vec256fe  const & x) {
    return Vec256ie(reinterpret_i(x.get_low()), reinterpret_i(x.get_high()));
}

static finline Vec256ie reinterpret_i (Vec256de const & x) {
    return Vec256ie(reinterpret_i(x.get_low()), reinterpret_i(x.get_high()));
}

static finline Vec256fe  reinterpret_f (Vec256ie const & x) {
    return Vec256fe(reinterpret_f(x.get_low()), reinterpret_f(x.get_high()));
}

static finline Vec256fe  reinterpret_f (Vec256fe  const & x) {
    return x;
}

static finline Vec256fe  reinterpret_f (Vec256de const & x) {
    return Vec256fe(reinterpret_f(x.get_low()), reinterpret_f(x.get_high()));
}

static finline Vec256de reinterpret_d (Vec256ie const & x) {
    return Vec256de(reinterpret_d(x.get_low()), reinterpret_d(x.get_high()));
}

static finline Vec256de reinterpret_d (Vec256fe  const & x) {
    return Vec256de(reinterpret_d(x.get_low()), reinterpret_d(x.get_high()));
}

static finline Vec256de reinterpret_d (Vec256de const & x) {
    return x;
}
...
template <int n>
static finline Vec2d select4(Vec4d const & a, Vec4d const & b) {
    switch (n) {
    case 0:
        return a.get_low();
    case 1:
...
// helper function used below
template <int n>
static finline Vec4f select4(Vec8f const & a, Vec8f const & b) {
    switch (n) {
    case 0:
        return a.get_low();
    case 1:
