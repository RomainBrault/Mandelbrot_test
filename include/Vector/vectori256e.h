...
public:
    ...
    finline Vec256b & load_a(void const * p) {
        y0 = _mm_load_si128((__m128i const*)p);
        y1 = _mm_load_si128((__m128i const*)p + 1);
        return *this;
    }
    finline Vec256b & load_s(void const * p) {
        __m128i temp1 = _mm_stream_load_si128(reinterpret_cast<__m128i*>(const_cast<void *>(p))  );
        __m128i temp2 = _mm_stream_load_si128(reinterpret_cast<__m128i*>(const_cast<void *>(p))+1);
        y0 = temp1;
        y1 = temp2;
        return *this;
    }
    // Member function to store into array (unaligned)
    finline void store(void * p) const {
    ...
    // divisible by 32, but there is hardly any speed advantage of load_a on modern processors
    finline void store_a(void * p) const {
        _mm_store_si128((__m128i*)p,     y0);
        _mm_store_si128((__m128i*)p + 1, y1);
    }
    finline void store_s(void * p) const {
    ...
    finline int get_bit(uint32_t index) const {
        if (index < 128) {
    ...
    finline bool operator [] (uint32_t index) const {
        return get_bit(index) != 0;
    }
    // Member functions to split into two Vec128b:
    finline Vec128b get_low() const {
        return y0;
    }
    finline Vec128b get_high() const {
        return y1;
    }
};
...
static finline Vec256b operator & (Vec256b const & a, Vec256b const & b) {
...
static finline Vec256b operator ~ (Vec256b const & a) {
    return Vec256b(~a.get_low(), ~a.get_high());
}

// vector operator &= : bitwise and
static finline Vec256b & operator &= (Vec256b & a, Vec256b const & b) {
    a = a & b;
    return a;
}
...
// function andnot: a & ~ b
static finline Vec256b andnot (Vec256b const & a, Vec256b const & b) {
...
static finline Vec256ie constant8i() {
    static const union {
        int32_t i[8];
        __m128i y[2];
...
static finline Vec256ie selectb (Vec256ie const & s, Vec256ie const & a, Vec256ie const & b) {
...
static finline bool horizontal_and (Vec256b const & a) {
    return horizontal_and(a.get_low() & a.get_high());
}

// horizontal_or. Returns true if at least one bit is 1
static finline bool horizontal_or (Vec256b const & a) {
...
public:
    ...
    // Constructor to broadcast the same value into all elements:
    finline Vec32c(char i) {
        y1 = y0 = _mm_set1_epi8(i);
    };
    // Constructor to build from all elements:
    finline Vec32c(int8_t i0, int8_t i1, int8_t i2, int8_t i3, int8_t i4, int8_t i5, int8_t i6, int8_t i7,
    ...
    finline Vec32c(Vec16c const & a0, Vec16c const & a1) {
        y0 = a0;  y1 = a1;
    }
    // Constructor to convert from type Vec256ie
    finline Vec32c(Vec256ie const & x) {
        y0 = x.get_low();
        y1 = x.get_high();
    };
    // Assignment operator to convert from type Vec256ie
    finline Vec32c & operator = (Vec256ie const & x) {
        y0 = x.get_low();
        y1 = x.get_high();
        return *this;
    };
    // Member function to load from array (unaligned)
    finline Vec32c & load(void const * p) {
        y0 = _mm_loadu_si128((__m128i const*)p);
        y1 = _mm_loadu_si128((__m128i const*)p + 1);
        return *this;
    }
    // Member function to load from array, aligned by 32
    finline Vec32c & load_a(void const * p) {
        y0 = _mm_load_si128((__m128i const*)p);
        y1 = _mm_load_si128((__m128i const*)p + 1);
        return *this;
    }
    finline Vec32c & load_s(void const * p) {
        __m128i temp1 = _mm_stream_load_si128(reinterpret_cast<__m128i*>(const_cast<void *>(p))  );
        __m128i temp2 = _mm_stream_load_si128(reinterpret_cast<__m128i*>(const_cast<void *>(p))+1);
    ...
    finline Vec32c & load_partial(int n, void const * p) {
        if (n <= 0) {
            *this = 0;
        }
        else if (n <= 16) {
    ...
    finline void store_partial(int n, void * p) const {
        if (n <= 0) {
            return;
        }
        else if (n <= 16) {
    ...
    finline Vec32c const & insert(uint32_t index, int8_t value) {
        if (index < 16) {
    ...
    finline int8_t extract(uint32_t index) const {
        if (index < 16) {
    ...
    // Extract a single element. Use store function if extracting more than one element.
    // Operator [] can only read an element, not write.
    finline int8_t operator [] (uint32_t index) const {
        return extract(index);
    }
    // Member functions to split into two Vec16c:
    finline Vec16c get_low() const {
        return y0;
    }
    finline Vec16c get_high() const {
        return y1;
    }
};
...
static finline Vec32c operator + (Vec32c const & a, Vec32c const & b) {
    return Vec32c(a.get_low() + b.get_low(), a.get_high() + b.get_high());
}

// vector operator += : add
static finline Vec32c & operator += (Vec32c & a, Vec32c const & b) {
    a = a + b;
    return a;
}

// postfix operator ++
static finline Vec32c operator ++ (Vec32c & a, int) {
    Vec32c a0 = a;
    a = a + 1;
    return a0;
}
...
// prefix operator ++
static finline Vec32c & operator ++ (Vec32c & a) {
    a = a + 1;
    return a;
}
...
static finline Vec32c operator - (Vec32c const & a) {
...
// vector of 32 8-bit signed integers
static finline Vec32c operator / (Vec32c const & a, Divisor_s const & d) {
    return Vec32c(a.get_low() / d, a.get_high() / d);
}

// vector operator /= : divide
static finline Vec32c & operator /= (Vec32c & a, Divisor_s const & d) {
    a = a / d;
    return a;
}

// vector operator << : shift left all elements
static finline Vec32c operator << (Vec32c const & a, int b) {
    return Vec32c(a.get_low() << b, a.get_high() << b);
}

// vector operator <<= : shift left
static finline Vec32c & operator <<= (Vec32c & a, int b) {
    a = a << b;
    return a;
}
...
public:
    ...
    // Constructor to broadcast the same value into all elements:
    finline Vec32uc(uint8_t i) {
        y1 = y0 = _mm_set1_epi8(i);
    };
    // Constructor to build from all elements:
    finline Vec32uc(uint8_t i0, uint8_t i1, uint8_t i2, uint8_t i3, uint8_t i4, uint8_t i5, uint8_t i6, uint8_t i7,
    ...
    finline Vec32uc(Vec16uc const & a0, Vec16uc const & a1) {
        y0 = a0;  y1 = a1;
    }
    // Constructor to convert from type Vec256ie
    finline Vec32uc(Vec256ie const & x) {
        y0 = x.get_low();  y1 = x.get_high();
    };
    // Assignment operator to convert from type Vec256ie
    finline Vec32uc & operator = (Vec256ie const & x) {
        y0 = x.get_low();  y1 = x.get_high();
        return *this;
    };
    // Member function to load from array (unaligned)
    finline Vec32uc & load(void const * p) {
        y0 = _mm_loadu_si128((__m128i const*)p);
        y1 = _mm_loadu_si128((__m128i const*)p + 1);
        return *this;
    }
    // Member function to load from array, aligned by 32
    finline Vec32uc & load_a(void const * p) {
        y0 = _mm_load_si128((__m128i const*)p);
        y1 = _mm_load_si128((__m128i const*)p + 1);
        return *this;
    }
    finline Vec32uc & load_s(void const * p) {
        __m128i temp1 = _mm_stream_load_si128(reinterpret_cast<__m128i*>(const_cast<void *>(p))  );
        __m128i temp2 = _mm_stream_load_si128(reinterpret_cast<__m128i*>(const_cast<void *>(p))+1);
        *reinterpret_cast< __m128i* >(&y0) = temp1;
        *reinterpret_cast< __m128i* >(&y1) = temp2;
    ...
    // Member function to change a single element in vector
    // Note: This function is inefficient. Use load function if changing more than one element
    finline Vec32uc const & insert(uint32_t index, uint8_t value) {
        Vec32c::insert(index, value);
        return *this;
    }
    // Member function extract a single element from vector
    finline uint8_t extract(uint32_t index) const {
        return Vec32c::extract(index);
    }
    // Extract a single element. Use store function if extracting more than one element.
    // Operator [] can only read an element, not write.
    finline uint8_t operator [] (uint32_t index) const {
        return extract(index);
    }
    // Member functions to split into two Vec16uc:
    finline Vec16uc get_low() const {
        return y0;
    }
    finline Vec16uc get_high() const {
        return y1;
    }
};
...
// vector operator + : add
static finline Vec32uc operator + (Vec32uc const & a, Vec32uc const & b) {
...
// vector operator / : divide
static finline Vec32uc operator / (Vec32uc const & a, Divisor_us const & d) {
    return Vec32uc(a.get_low() / d, a.get_high() / d);
}

// vector operator /= : divide
static finline Vec32uc & operator /= (Vec32uc & a, Divisor_us const & d) {
    a = a / d;
    return a;
}

// vector operator << : shift left all elements
static finline Vec32uc operator << (Vec32uc const & a, uint32_t b) {
    return Vec32uc(a.get_low() << b, a.get_high() << b);
}

// vector operator << : shift left all elements
static finline Vec32uc operator << (Vec32uc const & a, int32_t b) {
    return a << (uint32_t)b;
}
...
// vector operator >>= : shift right artihmetic
static finline Vec32uc & operator >>= (Vec32uc & a, uint32_t b) {
    a = a >> b;
    return a;
}

// vector operator >= : returns true for elements for which a >= b (unsigned)
static finline Vec32c operator >= (Vec32uc const & a, Vec32uc const & b) {
    return Vec32c(a.get_low() >= b.get_low(), a.get_high() >= b.get_high());
}
...
// vector operator ~ : bitwise not
static finline Vec32uc operator ~ (Vec32uc const & a) {
...
// Each byte in s must be either 0 (false) or -1 (true). No other values are allowed.
// (s is signed)
static finline Vec32uc select (Vec32c const & s, Vec32uc const & a, Vec32uc const & b) {
    return selectb(s,a,b);
}
...
static finline uint32_t horizontal_add (Vec32uc const & a) {
    return horizontal_add(a.get_low() + a.get_high());
}

// Horizontal add extended: Calculates the sum of all vector elements.
// Each element is zero-extended before addition to avoid overflow
static finline uint32_t horizontal_add_x (Vec32uc const & a) {
    return horizontal_add_x(a.get_low()) + horizontal_add_x(a.get_high());
}

// function add_saturated: add element by element, unsigned with saturation
static finline Vec32uc add_saturated(Vec32uc const & a, Vec32uc const & b) {
...
static finline Vec32uc sub_saturated(Vec32uc const & a, Vec32uc const & b) {
    return Vec32uc(sub_saturated(a.get_low(),b.get_low()), sub_saturated(a.get_high(),b.get_high()));
}

// function max: a > b ? a : b
static finline Vec32uc max(Vec32uc const & a, Vec32uc const & b) {
    return Vec32uc(max(a.get_low(),b.get_low()), max(a.get_high(),b.get_high()));
}

// function min: a < b ? a : b
static finline Vec32uc min(Vec32uc const & a, Vec32uc const & b) {
...
public:
    ...
    // Constructor to broadcast the same value into all elements:
    finline Vec16s(short i) {
        y1 = y0 = _mm_set1_epi16(i);
    };
    // Constructor to build from all elements:
    finline Vec16s(int16_t i0, int16_t i1, int16_t i2,  int16_t i3,  int16_t i4,  int16_t i5,  int16_t i6,  int16_t i7,
    ...
    finline Vec16s(Vec8s const & a0, Vec8s const & a1) {
        y0 = a0;  y1 = a1;
    }
    // Constructor to convert from type Vec256ie
    finline Vec16s(Vec256ie const & x) {
        y0 = x.get_low();  y1 = x.get_high();
    };
    // Assignment operator to convert from type Vec256ie
    finline Vec16s & operator = (Vec256ie const & x) {
        y0 = x.get_low();  y1 = x.get_high();
        return *this;
    };
    // Member function to load from array (unaligned)
    finline Vec16s & load(void const * p) {
        y0 = _mm_loadu_si128((__m128i const*)p);
        y1 = _mm_loadu_si128((__m128i const*)p + 1);
        return *this;
    }
    // Member function to load from array, aligned by 32
    finline Vec16s & load_a(void const * p) {
        y0 = _mm_load_si128((__m128i const*)p);
        y1 = _mm_load_si128((__m128i const*)p + 1);
        return *this;
    }
    finline Vec16s & load_s(void const * p) {
        __m128i temp1 = _mm_stream_load_si128(reinterpret_cast<__m128i*>(const_cast<void *>(p))  );
        __m128i temp2 = _mm_stream_load_si128(reinterpret_cast<__m128i*>(const_cast<void *>(p))+1);
        *reinterpret_cast< __m128i* >(&y0) = temp1;
        *reinterpret_cast< __m128i* >(&y1) = temp2;
    ...
    // Partial load. Load n elements and set the rest to 0
    finline Vec16s & load_partial(int n, void const * p) {
        if (n <= 0) {
            *this = 0;
        }
        else if (n <= 8) {
    ...
    finline Vec16s & cutoff(int n) {
        *this = Vec32c(*this).cutoff(n * 2);
        return *this;
    }
    // Member function to change a single element in vector
    // Note: This function is inefficient. Use load function if changing more than one element
    finline Vec16s const & insert(uint32_t index, int16_t value) {
        if (index < 8) {
    ...
    // Member function extract a single element from vector
    finline int16_t extract(uint32_t index) const {
        if (index < 8) {
    ...
    // Extract a single element. Use store function if extracting more than one element.
    // Operator [] can only read an element, not write.
    finline int16_t operator [] (uint32_t index) const {
        return extract(index);
    }
    // Member functions to split into two Vec8s:
    finline Vec8s get_low() const {
        return y0;
    }
    finline Vec8s get_high() const {
        return y1;
    }
};
...
// vector operator + : add element by element
static finline Vec16s operator + (Vec16s const & a, Vec16s const & b) {
    return Vec16s(a.get_low() + b.get_low(), a.get_high() + b.get_high());
}

// vector operator += : add
static finline Vec16s & operator += (Vec16s & a, Vec16s const & b) {
    a = a + b;
    return a;
}

// postfix operator ++
static finline Vec16s operator ++ (Vec16s & a, int) {
    Vec16s a0 = a;
    a = a + 1;
    return a0;
}
...
// prefix operator ++
static finline Vec16s & operator ++ (Vec16s & a) {
    a = a + 1;
    return a;
}
...
// vector operator - : unary minus
static finline Vec16s operator - (Vec16s const & a) {
...
static finline Vec16s operator / (Vec16s const & a, Divisor_s const & d) {
    return Vec16s(a.get_low() / d, a.get_high() / d);
}

// vector operator /= : divide
static finline Vec16s & operator /= (Vec16s & a, Divisor_s const & d) {
    a = a / d;
    return a;
}

// vector operator << : shift left
static finline Vec16s operator << (Vec16s const & a, int b) {
    return Vec16s(a.get_low() << b, a.get_high() << b);
}

// vector operator <<= : shift left
static finline Vec16s & operator <<= (Vec16s & a, int b) {
    a = a << b;
    return a;
}
...
// Each byte in s must be either 0 (false) or -1 (true). No other values are allowed.
// (s is signed)
static finline Vec16s select (Vec16s const & s, Vec16s const & a, Vec16s const & b) {
    return selectb(s,a,b);
}

// Horizontal add: Calculates the sum of all vector elements.
// Overflow will wrap around
static finline int32_t horizontal_add (Vec16s const & a) {
    return horizontal_add(a.get_low() + a.get_high());
}

// Horizontal add extended: Calculates the sum of all vector elements.
// Elements are sign extended before adding to avoid overflow
static finline int32_t horizontal_add_x (Vec16s const & a) {
    return horizontal_add_x(a.get_low()) + horizontal_add_x(a.get_high());
}

// function add_saturated: add element by element, signed with saturation
static finline Vec16s add_saturated(Vec16s const & a, Vec16s const & b) {
    return Vec16s(add_saturated(a.get_low(),b.get_low()), add_saturated(a.get_high(),b.get_high()));
}

// function sub_saturated: subtract element by element, signed with saturation
static finline Vec16s sub_saturated(Vec16s const & a, Vec16s const & b) {
    return Vec16s(sub_saturated(a.get_low(),b.get_low()), sub_saturated(a.get_high(),b.get_high()));
}

// function max: a > b ? a : b
static finline Vec16s max(Vec16s const & a, Vec16s const & b) {
    return Vec16s(max(a.get_low(),b.get_low()), max(a.get_high(),b.get_high()));
}

// function min: a < b ? a : b
static finline Vec16s min(Vec16s const & a, Vec16s const & b) {
    return Vec16s(min(a.get_low(),b.get_low()), min(a.get_high(),b.get_high()));
}

// function abs: a >= 0 ? a : -a
static finline Vec16s abs(Vec16s const & a) {
    return Vec16s(abs(a.get_low()), abs(a.get_high()));
}

// function abs_saturated: same as abs, saturate if overflow
static finline Vec16s abs_saturated(Vec16s const & a) {
    return Vec16s(abs_saturated(a.get_low()), abs_saturated(a.get_high()));
}

// function rotate_left all elements
// Use negative count to rotate right
static finline Vec16s rotate_left(Vec16s const & a, int b) {
...
public:
    ...
    // Constructor to broadcast the same value into all elements:
    finline Vec16us(uint16_t i) {
        y1 = y0 = _mm_set1_epi16(i);
    };
    // Constructor to build from all elements:
    finline Vec16us(uint16_t i0, uint16_t i1, uint16_t i2,  uint16_t i3,  uint16_t i4,  uint16_t i5,  uint16_t i6,  uint16_t i7,
    ...
    finline Vec16us(Vec8us const & a0, Vec8us const & a1) {
        y0 = a0;  y1 = a1;
    }
    // Constructor to convert from type Vec256ie
    finline Vec16us(Vec256ie const & x) {
        y0 = x.get_low();  y1 = x.get_high();
    };
    // Assignment operator to convert from type Vec256ie
    finline Vec16us & operator = (Vec256ie const & x) {
        y0 = x.get_low();  y1 = x.get_high();
        return *this;
    };
    // Member function to load from array (unaligned)
    finline Vec16us & load(void const * p) {
        y0 = _mm_loadu_si128((__m128i const*)p);
        y1 = _mm_loadu_si128((__m128i const*)p + 1);
        return *this;
    }
    // Member function to load from array, aligned by 32
    finline Vec16us & load_a(void const * p) {
        y0 = _mm_load_si128((__m128i const*)p);
        y1 = _mm_load_si128((__m128i const*)p + 1);
        return *this;
    }
    finline Vec16us & load_s(void const * p) {
        __m128i temp1 = _mm_stream_load_si128(reinterpret_cast<__m128i*>(const_cast<void *>(p))  );
        __m128i temp2 = _mm_stream_load_si128(reinterpret_cast<__m128i*>(const_cast<void *>(p))+1);
        *reinterpret_cast< __m128i* >(&y0) = temp1;
        *reinterpret_cast< __m128i* >(&y1) = temp2;
    ...
    // Member function to change a single element in vector
    // Note: This function is inefficient. Use load function if changing more than one element
    finline Vec16us const & insert(uint32_t index, uint16_t value) {
        Vec16s::insert(index, value);
        return *this;
    };
    // Member function extract a single element from vector
    finline uint16_t extract(uint32_t index) const {
        return Vec16s::extract(index);
    }
    // Extract a single element. Use store function if extracting more than one element.
    // Operator [] can only read an element, not write.
    finline uint16_t operator [] (uint32_t index) const {
        return extract(index);
    }
    // Member functions to split into two Vec8us:
    finline Vec8us get_low() const {
        return y0;
    }
    finline Vec8us get_high() const {
        return y1;
    }
};
...
// vector operator + : add
static finline Vec16us operator + (Vec16us const & a, Vec16us const & b) {
...
// vector operator / : divide
static finline Vec16us operator / (Vec16us const & a, Divisor_us const & d) {
    return Vec16us(a.get_low() / d, a.get_high() / d);
}

// vector operator /= : divide
static finline Vec16us & operator /= (Vec16us & a, Divisor_us const & d) {
    a = a / d;
    return a;
}

// vector operator >> : shift right logical all elements
static finline Vec16us operator >> (Vec16us const & a, uint32_t b) {
    return Vec16us(a.get_low() >> b, a.get_high() >> b);
}

// vector operator >> : shift right logical all elements
static finline Vec16us operator >> (Vec16us const & a, int b) {
    return a >> (uint32_t)b;
}

// vector operator >>= : shift right artihmetic
static finline Vec16us & operator >>= (Vec16us & a, uint32_t b) {
    a = a >> b;
    return a;
}
...
// vector operator << : shift left all elements
static finline Vec16us operator << (Vec16us const & a, int32_t b) {
    return a << (uint32_t)b;
}

// vector operator >= : returns true for elements for which a >= b (unsigned)
static finline Vec16s operator >= (Vec16us const & a, Vec16us const & b) {
    return Vec16s(a.get_low() >= b.get_low(), a.get_high() >= b.get_high());
}
...
// vector operator ~ : bitwise not
static finline Vec16us operator ~ (Vec16us const & a) {
...
// (s is signed)
static finline Vec16us select (Vec16s const & s, Vec16us const & a, Vec16us const & b) {
    return selectb(s,a,b);
}

// Horizontal add: Calculates the sum of all vector elements.
// Overflow will wrap around
static finline uint32_t horizontal_add (Vec16us const & a) {
    return horizontal_add(a.get_low() + a.get_high());
}

// Horizontal add extended: Calculates the sum of all vector elements.
// Each element is zero-extended before addition to avoid overflow
static finline uint32_t horizontal_add_x (Vec16us const & a) {
    return horizontal_add_x(a.get_low()) + horizontal_add_x(a.get_high());
}

// function add_saturated: add element by element, unsigned with saturation
static finline Vec16us add_saturated(Vec16us const & a, Vec16us const & b) {
    return Vec16us(add_saturated(a.get_low(),b.get_low()), add_saturated(a.get_high(),b.get_high()));
}

// function sub_saturated: subtract element by element, unsigned with saturation
static finline Vec16us sub_saturated(Vec16us const & a, Vec16us const & b) {
    return Vec16us(sub_saturated(a.get_low(),b.get_low()), sub_saturated(a.get_high(),b.get_high()));
}

// function max: a > b ? a : b
static finline Vec16us max(Vec16us const & a, Vec16us const & b) {
    return Vec16us(max(a.get_low(),b.get_low()), max(a.get_high(),b.get_high()));
}

// function min: a < b ? a : b
static finline Vec16us min(Vec16us const & a, Vec16us const & b) {
...
public:
    ...
    // Constructor to build from all elements:
    finline Vec8i(int32_t i0, int32_t i1, int32_t i2, int32_t i3, int32_t i4, int32_t i5, int32_t i6, int32_t i7) {
        y0 = _mm_setr_epi32(i0, i1, i2, i3);
        y1 = _mm_setr_epi32(i4, i5, i6, i7);
    }
    // Constructor to build from two Vec4i:
    finline Vec8i(Vec4i const & a0, Vec4i const & a1) {
        y0 = a0;  y1 = a1;
    }
    // Constructor to convert from type Vec256ie
    finline Vec8i(Vec256ie const & x) {
        y0 = x.get_low();  y1 = x.get_high();
    }
    // Assignment operator to convert from type Vec256ie
    finline Vec8i & operator = (Vec256ie const & x) {
        y0 = x.get_low();  y1 = x.get_high();
        return *this;
    }
    // Member function to load from array (unaligned)
    finline Vec8i & load(void const * p) {
        y0 = _mm_loadu_si128((__m128i const*)p);
        y1 = _mm_loadu_si128((__m128i const*)p + 1);
        return *this;
    }
    // Member function to load from array, aligned by 32
    finline Vec8i & load_a(void const * p) {
        y0 = _mm_load_si128((__m128i const*)p);
        y1 = _mm_load_si128((__m128i const*)p + 1);
        return *this;
    }
    finline Vec8i & load_s(void const * p) {
        __m128i temp1 = _mm_stream_load_si128(reinterpret_cast<__m128i*>(const_cast<void *>(p))  );
        __m128i temp2 = _mm_stream_load_si128(reinterpret_cast<__m128i*>(const_cast<void *>(p))+1);
        *reinterpret_cast< __m128i* >(&y0) = temp1;
        *reinterpret_cast< __m128i* >(&y1) = temp2;
    ...
    // Partial load. Load n elements and set the rest to 0
    finline Vec8i & load_partial(int n, void const * p) {
        if (n <= 0) {
            *this = 0;
        }
        else if (n <= 4) {
    ...
    finline Vec8i & cutoff(int n) {
        *this = Vec32c(*this).cutoff(n * 4);
        return *this;
    }
    // Member function to change a single element in vector
    // Note: This function is inefficient. Use load function if changing more than one element
    finline Vec8i const & insert(uint32_t index, int32_t value) {
        if (index < 4) {
    ...
    finline int32_t extract(uint32_t index) const {
        if (index < 4) {
    ...
    // Extract a single element. Use store function if extracting more than one element.
    // Operator [] can only read an element, not write.
    finline int32_t operator [] (uint32_t index) const {
        return extract(index);
    }
    // Member functions to split into two Vec4i:
    finline Vec4i get_low() const {
        return y0;
    }
    finline Vec4i get_high() const {
        return y1;
    }
};
...
// vector operator + : add element by element
static finline Vec8i operator + (Vec8i const & a, Vec8i const & b) {
    return Vec8i(a.get_low() + b.get_low(), a.get_high() + b.get_high());
}

// vector operator += : add
static finline Vec8i & operator += (Vec8i & a, Vec8i const & b) {
    a = a + b;
    return a;
}

// postfix operator ++
static finline Vec8i operator ++ (Vec8i & a, int) {
    Vec8i a0 = a;
    a = a + 1;
    return a0;
}
...
// prefix operator ++
static finline Vec8i & operator ++ (Vec8i & a) {
    a = a + 1;
    return a;
}
...
// vector operator - : unary minus
static finline Vec8i operator - (Vec8i const & a) {
...
// vector operator / : divide all elements by same integer
static finline Vec8i operator / (Vec8i const & a, Divisor_i const & d) {
    return Vec8i(a.get_low() / d, a.get_high() / d);
}

// vector operator /= : divide
static finline Vec8i & operator /= (Vec8i & a, Divisor_i const & d) {
    a = a / d;
    return a;
}

// vector operator << : shift left
static finline Vec8i operator << (Vec8i const & a, int32_t b) {
    return Vec8i(a.get_low() << b, a.get_high() << b);
}

// vector operator <<= : shift left
static finline Vec8i & operator <<= (Vec8i & a, int32_t b) {
    a = a << b;
    return a;
}
...
// Each byte in s must be either 0 (false) or -1 (true). No other values are allowed.
// (s is signed)
static finline Vec8i select (Vec8i const & s, Vec8i const & a, Vec8i const & b) {
    return selectb(s,a,b);
}

// Horizontal add: Calculates the sum of all vector elements.
// Overflow will wrap around
static finline int32_t horizontal_add (Vec8i const & a) {
    return horizontal_add(a.get_low() + a.get_high());
}

// Horizontal add extended: Calculates the sum of all vector elements.
// Elements are sign extended before adding to avoid overflow
static finline int64_t horizontal_add_x (Vec8i const & a) {
    return horizontal_add_x(a.get_low()) + horizontal_add_x(a.get_high());
}

// function add_saturated: add element by element, signed with saturation
static finline Vec8i add_saturated(Vec8i const & a, Vec8i const & b) {
    return Vec8i(add_saturated(a.get_low(),b.get_low()), add_saturated(a.get_high(),b.get_high()));
}

// function sub_saturated: subtract element by element, signed with saturation
static finline Vec8i sub_saturated(Vec8i const & a, Vec8i const & b) {
    return Vec8i(sub_saturated(a.get_low(),b.get_low()), sub_saturated(a.get_high(),b.get_high()));
}

// function max: a > b ? a : b
static finline Vec8i max(Vec8i const & a, Vec8i const & b) {
    return Vec8i(max(a.get_low(),b.get_low()), max(a.get_high(),b.get_high()));
}

// function min: a < b ? a : b
static finline Vec8i min(Vec8i const & a, Vec8i const & b) {
    return Vec8i(min(a.get_low(),b.get_low()), min(a.get_high(),b.get_high()));
}

// function abs: a >= 0 ? a : -a
static finline Vec8i abs(Vec8i const & a) {
    return Vec8i(abs(a.get_low()), abs(a.get_high()));
}

// function abs_saturated: same as abs, saturate if overflow
static finline Vec8i abs_saturated(Vec8i const & a) {
    return Vec8i(abs_saturated(a.get_low()), abs_saturated(a.get_high()));
}

// function rotate_left all elements
// Use negative count to rotate right
static finline Vec8i rotate_left(Vec8i const & a, int b) {
...
public:
    ...
    // Constructor to broadcast the same value into all elements:
    finline Vec8ui(uint32_t i) {
        y1 = y0 = _mm_set1_epi32(i);
    };
    // Constructor to build from all elements:
    finline Vec8ui(uint32_t i0, uint32_t i1, uint32_t i2, uint32_t i3, uint32_t i4, uint32_t i5, uint32_t i6, uint32_t i7) {
        y0 = _mm_setr_epi32(i0, i1, i2, i3);
        y1 = _mm_setr_epi32(i4, i5, i6, i7);
    };
    // Constructor to build from two Vec4ui:
    finline Vec8ui(Vec4ui const & a0, Vec4ui const & a1) {
        y0 = a0;  y1 = a1;
    }
    // Constructor to convert from type Vec256ie
    finline Vec8ui(Vec256ie const & x) {
        y0 = x.get_low();  y1 = x.get_high();
    };
    // Assignment operator to convert from type Vec256ie
    finline Vec8ui & operator = (Vec256ie const & x) {
        y0 = x.get_low();  y1 = x.get_high();
        return *this;
    };
    // Member function to load from array (unaligned)
    finline Vec8ui & load(void const * p) {
        y0 = _mm_loadu_si128((__m128i const*)p);
        y1 = _mm_loadu_si128((__m128i const*)p + 1);
        return *this;
    }
    // Member function to load from array, aligned by 32
    finline Vec8ui & load_a(void const * p) {
        y0 = _mm_load_si128((__m128i const*)p);
        y1 = _mm_load_si128((__m128i const*)p + 1);
        return *this;
    }
    finline Vec8ui & load_s(void const * p){
        __m128i temp1 = _mm_stream_load_si128(reinterpret_cast<__m128i*>(const_cast<void *>(p))  );
        __m128i temp2 = _mm_stream_load_si128(reinterpret_cast<__m128i*>(const_cast<void *>(p))+1);
        *reinterpret_cast< __m128i* >(&y0) = temp1;
        *reinterpret_cast< __m128i* >(&y1) = temp2;
    ...
    // Member function to change a single element in vector
    // Note: This function is inefficient. Use load function if changing more than one element
    finline Vec8ui const & insert(uint32_t index, uint32_t value) {
        Vec8i::insert(index, value);
        return *this;
    }
    // Member function extract a single element from vector
    finline uint32_t extract(uint32_t index) const {
        return Vec8i::extract(index);
    }
    // Extract a single element. Use store function if extracting more than one element.
    // Operator [] can only read an element, not write.
    finline uint32_t operator [] (uint32_t index) const {
        return extract(index);
    }
    // Member functions to split into two Vec4ui:
    finline Vec4ui get_low() const {
        return y0;
    }
    finline Vec4ui get_high() const {
        return y1;
    }
};
...
// vector operator + : add
static finline Vec8ui operator + (Vec8ui const & a, Vec8ui const & b) {
...
// vector operator / : divide all elements by same integer
static finline Vec8ui operator / (Vec8ui const & a, Divisor_ui const & d) {
    return Vec8ui(a.get_low() / d, a.get_high() / d);
}

// vector operator /= : divide
static finline Vec8ui & operator /= (Vec8ui & a, Divisor_ui const & d) {
    a = a / d;
    return a;
}

// vector operator >> : shift right logical all elements
static finline Vec8ui operator >> (Vec8ui const & a, uint32_t b) {
    return Vec8ui(a.get_low() >> b, a.get_high() >> b);
}

// vector operator >> : shift right logical all elements
static finline Vec8ui operator >> (Vec8ui const & a, int32_t b) {
    return a >> (uint32_t)b;
}

// vector operator >>= : shift right logical
static finline Vec8ui & operator >>= (Vec8ui & a, uint32_t b) {
    a = a >> b;
    return a;
}

// vector operator >>= : shift right logical
static finline Vec8ui & operator >>= (Vec8ui & a, int32_t b) {
    a = a >> b;
    return a;
}
...
// vector operator > : returns true for elements for which a > b (unsigned)
static finline Vec8i operator > (Vec8ui const & a, Vec8ui const & b) {
    return Vec8i(a.get_low() > b.get_low(), a.get_high() > b.get_high());
}
...
// vector operator ~ : bitwise not
static finline Vec8ui operator ~ (Vec8ui const & a) {
...
// Each word in s must be either 0 (false) or -1 (true). No other values are allowed.
// (s is signed)
static finline Vec8ui select (Vec8i const & s, Vec8ui const & a, Vec8ui const & b) {
    return selectb(s,a,b);
}

// Horizontal add: Calculates the sum of all vector elements.
// Overflow will wrap around
static finline uint32_t horizontal_add (Vec8ui const & a) {
    return horizontal_add((Vec8i)a);
}

// Horizontal add extended: Calculates the sum of all vector elements.
// Elements are zero extended before adding to avoid overflow
static finline uint64_t horizontal_add_x (Vec8ui const & a) {
    return horizontal_add_x(a.get_low()) + horizontal_add_x(a.get_high());
}

// function add_saturated: add element by element, unsigned with saturation
static finline Vec8ui add_saturated(Vec8ui const & a, Vec8ui const & b) {
    return Vec8ui(add_saturated(a.get_low(),b.get_low()), add_saturated(a.get_high(),b.get_high()));
}

// function sub_saturated: subtract element by element, unsigned with saturation
static finline Vec8ui sub_saturated(Vec8ui const & a, Vec8ui const & b) {
    return Vec8ui(sub_saturated(a.get_low(),b.get_low()), sub_saturated(a.get_high(),b.get_high()));
}

// function max: a > b ? a : b
static finline Vec8ui max(Vec8ui const & a, Vec8ui const & b) {
    return Vec8ui(max(a.get_low(),b.get_low()), max(a.get_high(),b.get_high()));
}

// function min: a < b ? a : b
static finline Vec8ui min(Vec8ui const & a, Vec8ui const & b) {
...
public:
    ...
    // Constructor to broadcast the same value into all elements:
    finline Vec4q(int64_t i) {
        y0 = y1 = Vec2q(i);
    }
    // Constructor to build from all elements:
    finline Vec4q(int64_t i0, int64_t i1, int64_t i2, int64_t i3) {
        y0 = Vec2q(i0,i1);
        y1 = Vec2q(i2,i3);
    }
    // Constructor to build from two Vec2q:
    finline Vec4q(Vec2q const & a0, Vec2q const & a1) {
        y0 = a0;  y1 = a1;
    }
    // Constructor to convert from type Vec256ie
    finline Vec4q(Vec256ie const & x) {
        y0 = x.get_low();  y1 = x.get_high();
    }
    // Assignment operator to convert from type Vec256ie
    finline Vec4q & operator = (Vec256ie const & x) {
        y0 = x.get_low();  y1 = x.get_high();
        return *this;
    }
    // Member function to load from array (unaligned)
    finline Vec4q & load(void const * p) {
        y0 = _mm_loadu_si128((__m128i const*)p);
        y1 = _mm_loadu_si128((__m128i const*)p + 1);
        return *this;
    }
    // Member function to load from array, aligned by 32
    finline Vec4q & load_a(void const * p) {
        y0 = _mm_load_si128((__m128i const*)p);
        y1 = _mm_load_si128((__m128i const*)p + 1);
        return *this;
    }
    finline Vec4q & load_s(void const * p) {
        __m128i temp1 = _mm_stream_load_si128(reinterpret_cast<__m128i*>(const_cast<void *>(p))  );
        __m128i temp2 = _mm_stream_load_si128(reinterpret_cast<__m128i*>(const_cast<void *>(p))+1);
        *reinterpret_cast< __m128i* >(&y0) = temp1;
        *reinterpret_cast< __m128i* >(&y1) = temp2;
    ...
    // Partial load. Load n elements and set the rest to 0
    finline Vec4q & load_partial(int n, void const * p) {
        if (n <= 0) {
            *this = 0;
        }
        else if (n <= 2) {
    ...
    // cut off vector to n elements. The last 8-n elements are set to zero
    finline Vec4q & cutoff(int n) {
        *this = Vec32c(*this).cutoff(n * 8);
        return *this;
    }
    // Member function to change a single element in vector
    // Note: This function is inefficient. Use load function if changing more than one element
    finline Vec4q const & insert(uint32_t index, int64_t value) {
        if (index < 2) {
    ...
    // Member function extract a single element from vector
    // Note: This function is inefficient. Use store function if extracting more than one element
    finline int64_t extract(uint32_t index) const {
        if (index < 2) {
    ...
    // Extract a single element. Use store function if extracting more than one element.
    // Operator [] can only read an element, not write.
    finline int64_t operator [] (uint32_t index) const {
        return extract(index);
    }
    // Member functions to split into two Vec2q:
    finline Vec2q get_low() const {
        return y0;
    }
    finline Vec2q get_high() const {
        return y1;
    }
};
...
// vector operator + : add element by element
static finline Vec4q operator + (Vec4q const & a, Vec4q const & b) {
    return Vec4q(a.get_low() + b.get_low(), a.get_high() + b.get_high());
}

// vector operator += : add
static finline Vec4q & operator += (Vec4q & a, Vec4q const & b) {
    a = a + b;
    return a;
}

// postfix operator ++
static finline Vec4q operator ++ (Vec4q & a, int) {
    Vec4q a0 = a;
    a = a + 1;
    return a0;
}
...
// prefix operator ++
static finline Vec4q & operator ++ (Vec4q & a) {
    a = a + 1;
    return a;
}
...
// vector operator - : unary minus
static finline Vec4q operator - (Vec4q const & a) {
...
// vector operator << : shift left
static finline Vec4q operator << (Vec4q const & a, int32_t b) {
    return Vec4q(a.get_low() << b, a.get_high() << b);
}

// vector operator <<= : shift left
static finline Vec4q & operator <<= (Vec4q & a, int32_t b) {
    a = a << b;
    return a;
}
...
// Each byte in s must be either 0 (false) or -1 (true). No other values are allowed.
// (s is signed)
static finline Vec4q select (Vec4q const & s, Vec4q const & a, Vec4q const & b) {
    return selectb(s,a,b);
}

// Horizontal add: Calculates the sum of all vector elements.
// Overflow will wrap around
static finline int64_t horizontal_add (Vec4q const & a) {
    return horizontal_add(a.get_low() + a.get_high());
}

// function max: a > b ? a : b
static finline Vec4q max(Vec4q const & a, Vec4q const & b) {
    return Vec4q(max(a.get_low(),b.get_low()), max(a.get_high(),b.get_high()));
}

// function min: a < b ? a : b
static finline Vec4q min(Vec4q const & a, Vec4q const & b) {
    return Vec4q(min(a.get_low(),b.get_low()), min(a.get_high(),b.get_high()));
}

// function abs: a >= 0 ? a : -a
static finline Vec4q abs(Vec4q const & a) {
    return Vec4q(abs(a.get_low()), abs(a.get_high()));
}

// function abs_saturated: same as abs, saturate if overflow
static finline Vec4q abs_saturated(Vec4q const & a) {
    return Vec4q(abs_saturated(a.get_low()), abs_saturated(a.get_high()));
}

// function rotate_left all elements
// Use negative count to rotate right
static finline Vec4q rotate_left(Vec4q const & a, int b) {
...
public:
    ...
    // Constructor to broadcast the same value into all elements:
    finline Vec4uq(uint64_t i) {
        y1 = y0 = Vec2q(i);
    };
    // Constructor to build from all elements:
    finline Vec4uq(uint64_t i0, uint64_t i1, uint64_t i2, uint64_t i3) {
        y0 = Vec2q(i0, i1);
        y1 = Vec2q(i2, i3);
    };
    // Constructor to build from two Vec2uq:
    finline Vec4uq(Vec2uq const & a0, Vec2uq const & a1) {
        y0 = a0;  y1 = a1;
    }
    // Constructor to convert from type Vec256ie
    finline Vec4uq(Vec256ie const & x) {
        y0 = x.get_low();  y1 = x.get_high();
    };
    // Assignment operator to convert from type Vec256ie
    finline Vec4uq & operator = (Vec256ie const & x) {
        y0 = x.get_low();  y1 = x.get_high();
        return *this;
    };
    // Member function to load from array (unaligned)
    finline Vec4uq & load(void const * p) {
        y0 = _mm_loadu_si128((__m128i const*)p);
        y1 = _mm_loadu_si128((__m128i const*)p + 1);
        return *this;
    }
    // Member function to load from array, aligned by 32
    finline Vec4uq & load_a(void const * p) {
        y0 = _mm_load_si128((__m128i const*)p);
        y1 = _mm_load_si128((__m128i const*)p + 1);
        return *this;
    }
    finline Vec4uq & load_s(void const *p) {
        __m128i temp1 = _mm_stream_load_si128(reinterpret_cast<__m128i*>(const_cast<void *>(p))  );
        __m128i temp2 = _mm_stream_load_si128(reinterpret_cast<__m128i*>(const_cast<void *>(p))+1);
        *reinterpret_cast< __m128i* >(&y0) = temp1;
        *reinterpret_cast< __m128i* >(&y1) = temp2;
    ...
    // Member function to change a single element in vector
    // Note: This function is inefficient. Use load function if changing more than one element
    finline Vec4uq const & insert(uint32_t index, uint64_t value) {
        Vec4q::insert(index, value);
        return *this;
    }
    // Member function extract a single element from vector
    finline uint64_t extract(uint32_t index) const {
        return Vec4q::extract(index);
    }
    // Extract a single element. Use store function if extracting more than one element.
    // Operator [] can only read an element, not write.
    finline uint64_t operator [] (uint32_t index) const {
        return extract(index);
    }
    // Member functions to split into two Vec2uq:
    finline Vec2uq get_low() const {
        return y0;
    }
    finline Vec2uq get_high() const {
        return y1;
    }
};
...
// vector operator + : add
static finline Vec4uq operator + (Vec4uq const & a, Vec4uq const & b) {
...
// vector operator >> : shift right logical all elements
static finline Vec4uq operator >> (Vec4uq const & a, uint32_t b) {
    return Vec4uq(a.get_low() >> b, a.get_high() >> b);
}

// vector operator >> : shift right logical all elements
static finline Vec4uq operator >> (Vec4uq const & a, int32_t b) {
    return a >> (uint32_t)b;
}

// vector operator >>= : shift right artihmetic
static finline Vec4uq & operator >>= (Vec4uq & a, uint32_t b) {
    a = a >> b;
    return a;
}
...
// vector operator > : returns true for elements for which a > b (unsigned)
static finline Vec4q operator > (Vec4uq const & a, Vec4uq const & b) {
    return Vec4q(a.get_low() > b.get_low(), a.get_high() > b.get_high());
}
...
// vector operator ~ : bitwise not
static finline Vec4uq operator ~ (Vec4uq const & a) {
...
// Each word in s must be either 0 (false) or -1 (true). No other values are allowed.
// (s is signed)
static finline Vec4q select (Vec4q const & s, Vec4uq const & a, Vec4uq const & b) {
    return selectb(s,a,b);
}

// Horizontal add: Calculates the sum of all vector elements.
// Overflow will wrap around
static finline uint64_t horizontal_add (Vec4uq const & a) {
    return horizontal_add((Vec4q)a);
}

// function max: a > b ? a : b
static finline Vec4uq max(Vec4uq const & a, Vec4uq const & b) {
    return Vec4uq(max(a.get_low(),b.get_low()), max(a.get_high(),b.get_high()));
}

// function min: a < b ? a : b
static finline Vec4uq min(Vec4uq const & a, Vec4uq const & b) {
...
static finline Vec4q permute4q(Vec4q const & a) {
    return Vec4q(blend2q<i0,i1> (a.get_low(), a.get_high()),
                 blend2q<i2,i3> (a.get_low(), a.get_high()));
}

template <int i0, int i1, int i2, int i3>
static finline Vec4uq permute4uq(Vec4uq const & a) {
...
static finline Vec16s permute16s(Vec16s const & a) {
...
template <int i0, int i1, int i2,  int i3,  int i4,  int i5,  int i6,  int i7,
          int i8, int i9, int i10, int i11, int i12, int i13, int i14, int i15 >
static finline Vec16us permute16us(Vec16us const & a) {
...
static finline Vec32c permute32c(Vec32c const & a) {
...
template <int i0,  int i1,  int i2,  int i3,  int i4,  int i5,  int i6,  int i7,
          ...
          int i16, int i17, int i18, int i19, int i20, int i21, int i22, int i23,
          int i24, int i25, int i26, int i27, int i28, int i29, int i30, int i31 >
    static finline Vec32uc permute32uc(Vec32uc const & a) {
...
template <int n>
static finline Vec2q select4(Vec4q const & a, Vec4q const & b) {
    switch (n) {
    case 0:
        return a.get_low();
    case 1:
...
// blend vectors Vec4q
template <int i0, int i1, int i2, int i3>
static finline Vec4q blend4q(Vec4q const & a, Vec4q const & b) {
...
template <int i0, int i1, int i2, int i3>
static finline Vec4uq blend4uq(Vec4uq const & a, Vec4uq const & b) {
    return Vec4uq( blend4q<i0,i1,i2,i3> (a,b));
}

// helper function used below
template <int n>
static finline Vec4i select4(Vec8i const & a, Vec8i const & b) {
    switch (n) {
    case 0:
        return a.get_low();
    case 1:
...
// blend vectors Vec8i
template <int i0, int i1, int i2, int i3, int i4, int i5, int i6, int i7>
static finline Vec8i blend8i(Vec8i const & a, Vec8i const & b) {
...
template <int i0, int i1, int i2, int i3, int i4, int i5, int i6, int i7>
static finline Vec8ui blend8ui(Vec8ui const & a, Vec8ui const & b) {
    return Vec8ui( blend8i<i0,i1,i2,i3,i4,i5,i6,i7> (a,b));
}

// helper function used below
template <int n>
static finline Vec8s select4(Vec16s const & a, Vec16s const & b) {
    switch (n) {
    case 0:
        return a.get_low();
    case 1:
...
template <int i0,  int i1,  int i2,  int i3,  int i4,  int i5,  int i6,  int i7,
          int i8,  int i9,  int i10, int i11, int i12, int i13, int i14, int i15 >
static finline Vec16s blend16s(Vec16s const & a, Vec16s const & b) {
...
template <int i0, int i1, int i2,  int i3,  int i4,  int i5,  int i6,  int i7,
          int i8, int i9, int i10, int i11, int i12, int i13, int i14, int i15 >
static finline Vec16us blend16us(Vec16us const & a, Vec16us const & b) {
    return Vec16us( blend16s<i0,i1,i2,i3,i4,i5,i6,i7,i8,i9,i10,i11,i12,i13,i14,i15> (a,b));
}

// helper function used below
template <int n>
static finline Vec16c select4(Vec32c const & a, Vec32c const & b) {
    switch (n) {
    case 0:
        return a.get_low();
    case 1:
...
template <int i0,  int i1,  int i2,  int i3,  int i4,  int i5,  int i6,  int i7,
          ...
          int i16, int i17, int i18, int i19, int i20, int i21, int i22, int i23,
          int i24, int i25, int i26, int i27, int i28, int i29, int i30, int i31 >
static finline Vec32c blend32c(Vec32c const & a, Vec32c const & b) {
...
template <
    ...
    int i16, int i17, int i18, int i19, int i20, int i21, int i22, int i23,
    int i24, int i25, int i26, int i27, int i28, int i29, int i30, int i31 >
    static finline Vec32uc blend32uc(Vec32uc const & a, Vec32uc const & b) {
...
static finline Vec32c lookup32(Vec32c const & index, Vec32c const & table) {
...
template <int n>
static finline Vec32c lookup(Vec32uc const & index, void const * table) {
    if (n <=  0) return 0;
    if (n <= 16) {
...
template <int n>
static finline Vec32c lookup(Vec32c const & index, void const * table) {
    return lookup<n>(Vec32uc(index), table);
}


static finline Vec16s lookup16(Vec16s const & index, Vec16s const & table) {
...
template <int n>
static finline Vec16s lookup(Vec16s const & index, void const * table) {
    if (n <=  0) return 0;
    if (n <=  8) {
...
static finline Vec8i lookup8(Vec8i const & index, Vec8i const & table) {
...
template <int n>
static finline Vec8i lookup(Vec8i const & index, void const * table) {
    if (n <= 0) return 0;
    if (n <= 4) {
...
static finline Vec4q lookup4(Vec4q const & index, Vec4q const & table) {
    return lookup8(Vec8i(index * 0x200000002ll + 0x100000000ll), Vec8i(table));
}

template <int n>
static finline Vec4q lookup(Vec4q const & index, void const * table) {
    if (n <= 0) return 0;
    // n > 0. Limit index
    Vec4uq index1;
    if ((n & (n-1)) == 0) {
...
static finline Vec32c shift_bytes_up(Vec32c const & a, int b) {
    if (b < 16) {
...
// You may use a permute function instead if b is a compile-time constant
static finline Vec32c shift_bytes_down(Vec32c const & a, int b) {
    if (b < 16) {
...
static finline Vec16s extend_low (Vec32c const & a) {
...
static finline Vec16s extend_high (Vec32c const & a) {
...
static finline Vec16us extend_low (Vec32uc const & a) {
...
static finline Vec16us extend_high (Vec32uc const & a) {
...
static finline Vec8i extend_low (Vec16s const & a) {
...
static finline Vec8i extend_high (Vec16s const & a) {
...
static finline Vec8ui extend_low (Vec16us const & a) {
...
static finline Vec8ui extend_high (Vec16us const & a) {
...
static finline Vec4q extend_low (Vec8i const & a) {
...
static finline Vec4q extend_high (Vec8i const & a) {
...
static finline Vec4uq extend_low (Vec8ui const & a) {
...
static finline Vec4uq extend_high (Vec8ui const & a) {
...
static finline Vec32c compress (Vec16s const & low, Vec16s const & high) {
    return Vec32c(compress(low.get_low(),low.get_high()), compress(high.get_low(),high.get_high()));
}

// Function compress : packs two vectors of 16-bit integers into one vector of 8-bit integers
// Signed, with saturation
static finline Vec32c compress_saturated (Vec16s const & low, Vec16s const & high) {
...
static finline Vec32uc compress (Vec16us const & low, Vec16us const & high) {
    return Vec32uc(compress(low.get_low(),low.get_high()), compress(high.get_low(),high.get_high()));
}

// Function compress : packs two vectors of 16-bit integers into one vector of 8-bit integers
// Unsigned, with saturation
static finline Vec32uc compress_saturated (Vec16us const & low, Vec16us const & high) {
...
// Overflow wraps around
static finline Vec16s compress (Vec8i const & low, Vec8i const & high) {
    return Vec16s(compress(low.get_low(),low.get_high()), compress(high.get_low(),high.get_high()));
}

// Function compress : packs two vectors of 32-bit integers into one vector of 16-bit integers
// Signed with saturation
static finline Vec16s compress_saturated (Vec8i const & low, Vec8i const & high) {
    return Vec16s(compress_saturated(low.get_low(),low.get_high()), compress_saturated(high.get_low(),high.get_high()));
}

// Function compress : packs two vectors of 32-bit integers into one vector of 16-bit integers
// Overflow wraps around
static finline Vec16us compress (Vec8ui const & low, Vec8ui const & high) {
    return Vec16us(compress(low.get_low(),low.get_high()), compress(high.get_low(),high.get_high()));
}

// Function compress : packs two vectors of 32-bit integers into one vector of 16-bit integers
// Unsigned, with saturation
static finline Vec16us compress_saturated (Vec8ui const & low, Vec8ui const & high) {
...
// Overflow wraps around
static finline Vec8i compress (Vec4q const & low, Vec4q const & high) {
    return Vec8i(compress(low.get_low(),low.get_high()), compress(high.get_low(),high.get_high()));
}

// Function compress : packs two vectors of 64-bit integers into one vector of 32-bit integers
// Signed, with saturation
static finline Vec8i compress_saturated (Vec4q const & low, Vec4q const & high) {
    return Vec8i(compress_saturated(low.get_low(),low.get_high()), compress_saturated(high.get_low(),high.get_high()));
}

// Function compress : packs two vectors of 32-bit integers into one vector of 16-bit integers
// Overflow wraps around
static finline Vec8ui compress (Vec4uq const & low, Vec4uq const & high) {
    return Vec8ui (compress((Vec4q)low, (Vec4q)high));
}

// Function compress : packs two vectors of 64-bit integers into one vector of 32-bit integers
// Unsigned, with saturation
static finline Vec8ui compress_saturated (Vec4uq const & low, Vec4uq const & high) {
...
template <int32_t d>
static finline Vec8i divide_by_i(Vec8i const & a) {
    return Vec8i( divide_by_i<d>(a.get_low()), divide_by_i<d>(a.get_high()));
}

// define Vec8i a / const_int(d)
template <int32_t d>
static finline Vec8i operator / (Vec8i const & a, Const_int_t<d>) {
    return divide_by_i<d>(a);
}

// define Vec8i a / const_uint(d)
template <uint32_t d>
static finline Vec8i operator / (Vec8i const & a, Const_uint_t<d>) {
...
// vector operator /= : divide
template <int32_t d>
static finline Vec8i & operator /= (Vec8i & a, Const_int_t<d> b) {
    a = a / b;
    return a;
}
...
// vector operator /= : divide
template <uint32_t d>
static finline Vec8i & operator /= (Vec8i & a, Const_uint_t<d> b) {
    a = a / b;
    return a;
}
...
template <uint32_t d>
static finline Vec8ui divide_by_ui(Vec8ui const & a) {
    return Vec8ui( divide_by_ui<d>(a.get_low()), divide_by_ui<d>(a.get_high()));
}

// define Vec8ui a / const_uint(d)
template <uint32_t d>
static finline Vec8ui operator / (Vec8ui const & a, Const_uint_t<d>) {
    return divide_by_ui<d>(a);
}

// define Vec8ui a / const_int(d)
template <int32_t d>
static finline Vec8ui operator / (Vec8ui const & a, Const_int_t<d>) {
...
// vector operator /= : divide
template <uint32_t d>
static finline Vec8ui & operator /= (Vec8ui & a, Const_uint_t<d> b) {
    a = a / b;
    return a;
}
...
// vector operator /= : divide
template <int32_t d>
static finline Vec8ui & operator /= (Vec8ui & a, Const_int_t<d> b) {
    a = a / b;
    return a;
}
...
template <int d>
static finline Vec16s divide_by_i(Vec16s const & a) {
    return Vec16s( divide_by_i<d>(a.get_low()), divide_by_i<d>(a.get_high()));
}

// define Vec16s a / const_int(d)
template <int d>
static finline Vec16s operator / (Vec16s const & a, Const_int_t<d>) {
    return divide_by_i<d>(a);
}

// define Vec16s a / const_uint(d)
template <uint32_t d>
static finline Vec16s operator / (Vec16s const & a, Const_uint_t<d>) {
...
// vector operator /= : divide
template <int32_t d>
static finline Vec16s & operator /= (Vec16s & a, Const_int_t<d> b) {
    a = a / b;
    return a;
}
...
// vector operator /= : divide
template <uint32_t d>
static finline Vec16s & operator /= (Vec16s & a, Const_uint_t<d> b) {
    a = a / b;
    return a;
}
...
template <uint32_t d>
static finline Vec16us divide_by_ui(Vec16us const & a) {
    return Vec16us( divide_by_ui<d>(a.get_low()), divide_by_ui<d>(a.get_high()));
}

// define Vec16us a / const_uint(d)
template <uint32_t d>
static finline Vec16us operator / (Vec16us const & a, Const_uint_t<d>) {
    return divide_by_ui<d>(a);
}

// define Vec16us a / const_int(d)
template <int d>
static finline Vec16us operator / (Vec16us const & a, Const_int_t<d>) {
    Static_error_check< (d>=0) > Error_dividing_unsigned_by_negative;// Error: dividing unsigned by negative is ambiguous
    return divide_by_ui<d>(a);                                       // unsigned divide
}
...
// vector operator /= : divide
template <uint32_t d>
static finline Vec16us & operator /= (Vec16us & a, Const_uint_t<d> b) {
    a = a / b;
    return a;
}
...
// vector operator /= : divide
template <int32_t d>
static finline Vec16us & operator /= (Vec16us & a, Const_int_t<d> b) {
    a = a / b;
    return a;
}
...
template <int d>
static finline Vec32c operator / (Vec32c const & a, Const_int_t<d>) {
...
template <uint32_t d>
static finline Vec32c operator / (Vec32c const & a, Const_uint_t<d>) {
...
// vector operator /= : divide
template <int32_t d>
static finline Vec32c & operator /= (Vec32c & a, Const_int_t<d> b) {
    a = a / b;
    return a;
}
// vector operator /= : divide
template <uint32_t d>
static finline Vec32c & operator /= (Vec32c & a, Const_uint_t<d> b) {
    a = a / b;
    return a;
}
...
template <uint32_t d>
static finline Vec32uc operator / (Vec32uc const & a, Const_uint_t<d>) {
...
template <int d>
static finline Vec32uc operator / (Vec32uc const & a, Const_int_t<d>) {
...
// vector operator /= : divide
template <uint32_t d>
static finline Vec32uc & operator /= (Vec32uc & a, Const_uint_t<d> b) {
    a = a / b;
    return a;
}
...
// vector operator /= : divide
template <int32_t d>
static finline Vec32uc & operator /= (Vec32uc & a, Const_int_t<d> b) {
    a = a / b;
    return a;
}
